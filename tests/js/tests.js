...
require(['js/libs/requirejs.domReady', 'js/libs/createjs-2013.05.14.min', 'js/classes/Ball', 'js/classes/GravityMiniExample'], function(domReady, createjs, Ball, GravityMiniExample) {
	...
	domReady(function() {
		...
		test("GravityMiniExample class", function() {
			...
			equal( result.debugMode, false, "GravityMiniExample(), debugMode == false" );
			equal( result.width, 320, "GravityMiniExample(), width == 320" );
			equal( result.height, 240, "GravityMiniExample(), height == 240" );
			// Test properties on GravityMiniExample(true, 800, 600).
			result = new GravityMiniExample(true, 800, 600);
			deepEqual( typeof(result), 'object', "typeof(GravityMiniExample()) == 'object'" );
			equal( result.debugMode, true, "GravityMiniExample(), debugMode == true" );
			equal( result.width, 800, "GravityMiniExample(), width == 800" );
			equal( result.height, 600, "GravityMiniExample(), height == 600" );
		});
	});
